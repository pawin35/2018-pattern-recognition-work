...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    #############################################################################
    q = X.dot(W1) +b1.reshape(1,-1)
    r = np.maximum(q,0)
    scores = r.dot(W2) + b2.reshape(1,-1)
    #############################################################################
    #                              END OF TODO#1                                #
    #############################################################################
    ...
    # classifier loss.                                                          #
    #############################################################################
    sf = np.exp(scores - np.max(scores))
    p = sf / np.sum(sf, axis=1).reshape(-1,1)
    cross_beforesum = -np.log(p[range(N), y])
    cross_e = np.sum(cross_beforesum)/N
    l2_l = (np.sum(np.square(W1))+np.sum(np.square(W2)))*reg/2
    loss = cross_e + l2_l
    #############################################################################
    #                              END OF TODO#2                                #
    #############################################################################
    ...
    #############################################################################
    normal_prob = p.copy()
    normal_prob[range(N), y] -= 1
    ds = normal_prob
    dw2 = r.T.dot(ds)/N + reg*W2
    db2 = np.sum(ds, axis=0)/N
    mask = np.zeros_like(r)
    mask[r>0] = 1
    dr = ds.dot(W2.T) * mask
    dw1 = X.T.dot(dr)/N + reg*W1
    db1 = np.sum(dr, axis=0)/N
    grads['W1'] = dw1
    grads['W2'] = dw2
    grads['b1'] = db1
    grads['b2'] = db2
  ...
  def train(self, X, y, X_val, y_val,
    ...
    for it in range(num_iters):
      ...
      if N < batch_size:
        ...
        rand_idx = np.random.choice(N, batch_size, replace=False)
        X_batch = X[rand_idx]
        y_batch = y[rand_idx]
      #########################################################################
      #                             END OF YOUR TODO#4                        #
